    ...
    override func spec() {
        describe("A FabricConfig") {
            it("should set the skip flag if fabric json is as default") {
                ...
                let config = FabricConfig(filePath: bundle.pathForResource("fabric_invalid", ofType: "json")!)
            ...
            it("should unset the skip flag if fabric json is set") {
                let bundle = NSBundle(forClass: SpecHelper.self)
                let config = FabricConfig(filePath: bundle.pathForResource("fabric_valid", ofType: "json")!)
